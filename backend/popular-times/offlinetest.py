...
'''
userPlaceStorage = pt.get_id('AIzaSyDIYw-pp8I9Ivq6On4vasbUcGj5wi8e_Gc', userPlaceID)

userTypes = userPlaceStorage['types'] 
for a in userTypes:#We need to remove some categories since they are too prevalent, and would result in thousands of results.
    if a == 'health':
        userTypes.remove('health')
    ...
    if a == 'street_address':
        userTypes.remove('street_address')# categories = ['grocery_or_supermarket', 'pharmacy', 'bakery', 'food', 'health', etc] #We need to exclude some categories, including 'health', 'point_of_interest', 'store', 'establishment' since these are too broad and will yield too many results
placeStorage = pt.get("AIzaSyDIYw-pp8I9Ivq6On4vasbUcGj5wi8e_Gc", [userTypes], (coord1, coord2), (coord3, coord4)) #pt.get a list of businesses with the same user specified type and within a user specified radius from their location. 
singleUserTypes = str(userTypes[0])
#placeStorage = pt.get("AIzaSyDIYw-pp8I9Ivq6On4vasbUcGj5wi8e_Gc", [singleUserTypes], (coord1, coord2), (coord3, coord4)) #pt.get a list of businesses with the same user specified type and within a user specified radius from their location. 
print (placeStorage) #We can skip all of this since it just discovers similar places. We already put all of the similar places into placeStorage, so this is only necessary for the online version.'''

def popularAppend(): #placeStorage, placeID, name, address, types, current_popularity, international_phone_number
    global placeID
    global name
    global address
    global types
